...
<template>
  <aside class="w-full flex flex-col gap-4 select-none animate-fade-in">
    ...
    <div class="w-full rounded-2xl bg-white dark:bg-[#1a1a2e]/60 border border-gray-100 dark:border-white/[0.05] p-3 shadow-sm backdrop-blur-md transition-all duration-300 min-h-[340px] flex flex-col justify-between">
      ...
      >
        <div class="absolute inset-0 transition-all duration-300 pointer-events-none bg-black/0 group-hover:bg-black/30 group-active:bg-black/50 w-full h-full z-20 flex items-center justify-center">
          <svg 
            class="transition-all duration-300 transform scale-90 opacity-0 group-hover:scale-100 group-hover:opacity-100 text-white w-12 h-12" 
      ...
      <div class="px-2">
        <div class="font-bold text-xl text-center mb-1 text-gray-800 dark:text-neutral-50 transition-colors duration-300">
          {{ siteConfigStore.siteName }}
        </div>
        
        <div class="h-1 w-5 bg-primary mx-auto rounded-full mb-2 opacity-80 transition-all"></div>
        
        <div class="text-center text-sm text-neutral-400 mb-2.5 font-normal leading-relaxed max-w-[240px] mx-auto">
        ...
        <div v-if="socialLinksStore.hasSocialLinks" class="flex flex-wrap gap-2 justify-center mb-1">
          ...
          >
            <svg v-if="link.icon === 'github'" class="w-[1.5rem] h-[1.5rem]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
            <svg v-else-if="link.icon === 'email'" class="w-[1.5rem] h-[1.5rem]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
