...
<template>
  <aside aria-label="作者名片与博客导航" class="w-full flex flex-col gap-4 select-none animate-fade-in">
      ...
      <div class="px-2">
        ...
        <nav v-if="socialLinksStore.hasSocialLinks" aria-label="社交链接" class="flex flex-wrap gap-2 justify-center mb-1">
          ...
          >
            ...
            <svg v-else-if="link.icon === 'email'" aria-hidden="true" class="w-[1.5rem] h-[1.5rem]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
    ...
    <section v-if="announcements.length > 0" class="w-full rounded-2xl bg-white dark:bg-[#1a1a2e]/60 border border-gray-100 dark:border-white/[0.05] p-4 shadow-sm backdrop-blur-md transition-all duration-300">
      ...
      <div class="space-y-3">
        <article v-for="announcement in announcements" :key="announcement.id" class="group">
