...
renderer.code = (code: string, infostring: string | undefined, _escaped: boolean) => {
  ...
  return `<div class="code-block-wrapper relative group">
    <div class="absolute top-2 right-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity z-10">
      <button class="copy-code-btn p-1.5 bg-dark-200 dark:bg-dark-300 rounded text-gray-400 hover:text-primary transition-colors" data-code="${encodedCode}">${copyIcon}</button>
    </div>
