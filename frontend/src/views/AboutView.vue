...
// 🚀 核心新增：动态计算需要展示的模块以及它们的排序
const sortedSections = computed(() => {
  if (!profile.value) return []

  // 将各个模块的显隐开关（show_xxx）与排序字段（order_xxx）进行绑定
  const sections = [
    {
      id: 'tech_stack',
      show: profile.value.show_tech_stack ?? true,
      order: profile.value.order_tech_stack ?? 1
    },
    {
      id: 'journey',
      show: profile.value.show_journey ?? true,
      order: profile.value.order_journey ?? 2
    },
    {
      id: 'education',
      show: profile.value.show_education ?? true,
      order: profile.value.order_education ?? 3
    },
    {
      id: 'exploration',
      show: profile.value.show_exploration ?? true,
      order: profile.value.order_exploration ?? 4
    },
    {
      id: 'social',
      show: profile.value.show_social ?? true,
      order: profile.value.order_social ?? 5
    }
  ]

  // 过滤掉未开启的模块，并根据 order 升序排序（数字越小越靠前）
  return sections
    ...
    <div class="flex-1 min-w-0 lg:order-2">
      ...
      <template v-else-if="profile">
        ...
        <template v-for="section in sortedSections" :key="section.id">
          ...
          <div
            v-if="section.id === 'tech_stack' && profile.tech_stack?.length"
          ...
          <div
            v-if="section.id === 'journey' && profile.journey?.length"
            class="glass-card p-6 mb-6"
          >
            <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <svg
          ...
          <div
            v-if="section.id === 'education' && profile.education"
            class="glass-card p-6 mb-6"
          >
            <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <svg
          ...
          <div
            v-if="section.id === 'exploration' && profile.exploration_areas?.length"
            class="glass-card p-6 mb-6"
          >
            <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <svg
          ...
          <div
            v-if="section.id === 'social' && socialLinksStore.hasSocialLinks"
